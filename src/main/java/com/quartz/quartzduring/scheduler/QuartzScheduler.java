...
public class QuartzScheduler {
    ...
    private void startJob1(Scheduler scheduler) throws SchedulerException {
        ...
        JobDetail jobDetail = JobBuilder.newJob(HiJob.class).withIdentity("job1", "group1").build();
        // 基于表达式构建触发器
    ...
    private void startJob2(Scheduler scheduler) throws SchedulerException {
        // 通过JobBuilder构建JobDetail实例，JobDetail规定其job只能是实现Job接口的实例
        JobDetail jobDetail = JobBuilder.newJob(HiJob.class).withIdentity("job2", "group2").build();
        // 基于表达式构建触发器
        CronScheduleBuilder cronScheduleBuilder = CronScheduleBuilder.cronSchedule("0/5 * * * * ?");
        // CronTrigger表达式触发器 继承于Trigger。TriggerBuilder 用于构建触发器实例
